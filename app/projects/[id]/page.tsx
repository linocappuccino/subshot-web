...
export default function ProjectDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  function handleSceneDragEnd(event: DragEndEvent) {
    ...
    // 2026-09-09, Lino: "das EINZIGE was anders ist, ist die reihenfolge
    // in der shot reihenfolge ansonsten IST ALLES GLEICH" — same
    // indicator-driven insert-before/after as the narrative reorder below,
    // just against Scene.shooting_order (scoped to this ONE open section —
    // there's no cross-section concept in this mode) instead of
    // Scene.sort_order/section_id, so it can't reuse computeSceneReorder
    // (that one's specifically about the narrative field + moving between
    // sections). "Ohne Abschnitt" never reaches here — the toggle that
    // sets shotOrderMode only ever renders for a real section.
    if (shotOrderMode && originScene.section_id) {
      const sectionScenes = scenesInShootingOrder(originScene.section_id);
      const activeSceneObj = sectionScenes.find((s) => s.id === activeId);
      if (!overIdStr || !activeSceneObj || !sectionScenes.some((s) => s.id === overIdStr)) return;
      const without = sectionScenes.filter((s) => s.id !== activeId);
      const targetIdx = without.findIndex((s) => s.id === overIdStr);
      const insertAt = insertAfter ? targetIdx + 1 : targetIdx;
      const reordered = [...without];
      reordered.splice(insertAt, 0, activeSceneObj);
      const orderedSceneIds = reordered.map((s) => s.id);
      updateScenesShots((d) => ({
        ...d,
        scenes: d.scenes.map((s) => {
          const idx2 = orderedSceneIds.indexOf(s.id);
          return idx2 === -1 ? s : { ...s, shooting_order: idx2 };
        }),
      }));
      api.reorderScenesShootingOrder(originScene.section_id, orderedSceneIds).catch((e) => {
        toast.showError(e instanceof ApiError ? e.message : "Sortieren fehlgeschlagen.");
      });
      return;
    }
  ...
  // 2026-09-09, Lino: "es braucht doch auch genau dieselbe indikator und
  // drag animation etc. wie bei der szenen übersicht GENAU 1:1 gleich!!
  // das EINZIGE was anders ist, ist die reihenfolge" — a prior attempt
  // rebuilt the whole drag interaction (its own DndContext, indicator
  // state, onDragOver) for the Shot-Reihenfolge, which drifted from the
  // real one in feel (autoScroll tuning, the scroll-behavior throttle
  // suspend, the floating DragOverlay clone). Fixed at the root instead:
  // Shot-Reihenfolge now shares the EXACT SAME DndContext/handlers as the
  // Szenen-Reihenfolge (see their own onDragEnd's shotOrderMode branch) —
  // the only difference is which order this returns.
  const scenesInShootingOrder = (sectionId: string | null) =>
    [...scenesIn(sectionId)].sort((a, b) => {
      ...
      >
        ...
        >
          ...
          {sections.filter((section) => section.id === openSectionId).map((section) => {
            ...
            // 2026-09-09, Lino: "es braucht doch auch genau dieselbe
            // indikator und drag animation etc. wie bei der szenen
            // uebersicht GENAU 1:1 gleich!! das EINZIGE was anders ist,
            // ist die reihenfolge" -- Shot-Reihenfolge reuses this EXACT
            // same SectionBlock render (DndContext, indicator, DragOverlay,
            // header, menu -- everything below is unconditional now), only
            // the scene order fed into it differs. See handleSceneDragEnd's
            // own shotOrderMode branch for where a drop actually persists to.
            const group = shotOrderMode ? scenesInShootingOrder(section.id) : scenesIn(section.id);
            return (
              <SectionBlock
                key={section.id}
                section={section}
